...
# Assuming $customGroupedData is already sorted by kBereichT and Typ

$lastKBereichT = $null

foreach ($group in $customGroupedData) {
    $splitGroup = $group.Name -split ', '
    $kBereichT = $splitGroup[0]
    $typ = $splitGroup[1]
    $fernGewirktCount = ($group.Group | Where-Object { $_.FWSatzNr -ne '0' }).Count

    # Check if we're still in the same kBereichT group
    if ($lastKBereichT -ne $kBereichT) {
        if ($lastKBereichT -ne $null) {
            Write-Host "`n" # add a newline for separation between groups
        }
        Write-Host "Gruppe: $kBereichT -"
        $lastKBereichT = $kBereichT
    }

    # Now write the Typ line
    Write-Host "`tTyp: $typ - Total: $($group.Count) - Ferngewirkt: $fernGewirktCount"
}